  ...
  return (
    <Box sx={{ p: 3 }}>
      <Box sx={{ display: 'flex', gap: 2, alignItems: 'center', mb: 2 }}>
          ...
          onChange={async (e) => {
            ...
            try {
              ...
              if (name.endsWith('.csv') || name.endsWith('.txt')) {
                ...
                // try to dynamically import xlsx (SheetJS). If not available, instruct the user.
                try {
              ...
              // simple CSV parse (header row expected)
              ...
              for (const r of rows) {
                ...
                try {
                  const res = await addClient({
                    ...
                    email: r.email,
                    phone: r.phone || '',
                    companyId: Number(r.companyid) || 1,
                  });
                  if ((res as any).error) {
                    failures.push({ row: rowIndex, reason: (res as any).error, raw: JSON.stringify(r) });
                  } else {
                    success++;
                  }
                } catch (err: any) {
        ...
        <label htmlFor="clients-import-input">
      ...
      <Box sx={{ mb: 2, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
          ...
          <FormControl size="small" sx={{ minWidth: 160 }}>
            ...
            <Select
              labelId="pk-filter-label"
              value={pkFilter}
              label="Primary Key"
              onChange={(e) => {
                const val = e.target.value as unknown as string;
                setPkFilter(val === "" ? "" : Number(val));
              }}
              sx={{ color: '#000', backgroundColor: '#fff' }}
            >
              <MenuItem value="">All</MenuItem>
              {clients.map((c) => (
                <MenuItem key={c.clientid} value={c.clientid}>{String(c.clientid)}</MenuItem>
              ))}
            </Select>
          </FormControl>
          ...
          <FormControl size="small" sx={{ minWidth: 160 }}>
            ...
            <Select
              labelId="fk-filter-label"
              value={fkFilter}
              label="Foreign Key (companyId)"
              onChange={(e) => {
                const val = e.target.value as unknown as string;
                setFkFilter(val === "" ? "" : Number(val));
              }}
              sx={{ color: '#000', backgroundColor: '#fff' }}
            >
              <MenuItem value="">All</MenuItem>
              {[...new Set(clients.map((c) => c.companyid))].map((compId) => (
                <MenuItem key={compId} value={compId}>{String(compId)}</MenuItem>
              ))}
            </Select>
          </FormControl>
        </Box>
      ...
      <TableContainer component={Paper} sx={{ mb: 3 }}>
        <Table>
          <TableHead>
            <TableRow>
              <TableCell padding="checkbox">
                <Checkbox
                  indeterminate={selectedIds.length > 0 && selectedIds.length < clients.length}
                  checked={clients.length > 0 && selectedIds.length === clients.length}
                  onChange={(e) => {
                    if (e.target.checked) setSelectedIds(clients.map((c) => c.clientid));
                    else setSelectedIds([]);
                  }}
                />
              </TableCell>
          ...
          <TableBody>
            {clients
              .filter((c) => (pkFilter === "" ? true : c.clientid === pkFilter))
              .filter((c) => (fkFilter === "" ? true : c.companyid === fkFilter))
              .map((c) => (
              <TableRow
                key={c.clientid}
                hover
              ...
              >
                <TableCell padding="checkbox" onClick={(e)=>e.stopPropagation()}>
                  <Checkbox
                    checked={selectedIds.includes(c.clientid)}
                    onChange={(e)=>{
                      const checked = e.target.checked;
                      setSelectedIds((prev)=> checked ? [...prev, c.clientid] : prev.filter(id=>id!==c.clientid));
                    }}
                  />
                </TableCell>
      ...
      <Dialog open={confirmBulkOpen} onClose={() => setConfirmBulkOpen(false)}>
        ...
        <DialogContent>
          <Typography>Are you sure you want to delete {selectedIds.length} selected clients? This action cannot be undone.</Typography>
        </DialogContent>
        <DialogActions>
          ...
          <Button color="error" onClick={async ()=>{
            ...
            if ((res as any).error) {
              ...
              setToast({ open: true, message: `Deleted ${res.deleted || selectedIds.length} clients`, severity: 'success' });
              setSelectedIds([]);
              await loadClients(1);
            }
          }}>Delete</Button>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'center', mb: 3 }}>
        <Pagination
          ...
          onChange={(_, value) => {
            setPage(value);
            // reload clients for the new page
            loadClients(value);
          }}
          sx={{
